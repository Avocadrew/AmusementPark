...
//===========================================================================
{
	if (tw->runButton->value()) {	// only advance time if appropriate
		if (clock() - lastRedraw > CLOCKS_PER_SEC/30) {
			lastRedraw = clock();
			tw->trainView->waveTime += tw->speed->value();
			if (tw->trainView->waveTime > 2e20)
			{
				tw->trainView->waveTime = 0.0f;
			}
			//std::cout << tw->trainView->waveTime << std::endl;
			tw->trainView->heightMapIndex += tw->speed->value() / 1;
			if (tw->trainView->heightMapIndex > 199) tw->trainView->heightMapIndex -= 200;
			tw->advanceTrain();
			tw->damageMe();
		}
	}
